...
\[\epsilon_y : \fib_{p_g}(x)\ \simeq\ (g(x)=*)\]
But this is part of the Grothendieck correspondence. Then we need $\epsilon_*((*_X,*_g),\refl_{*_X}) = *_g$, but the equivalence from the Grothendieck correspondance is defined by transporting the second component $*_g$ along the third $\refl_{*_X}$. So we can conclude.
\end{proof}
