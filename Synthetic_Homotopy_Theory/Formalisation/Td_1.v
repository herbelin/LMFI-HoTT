...
Lemma stupid {A B} (f : A -> B)(g : B -> A) :  (f ¤ g)  =~ Id B -> (f ¤ g) = Id B.
Proof.
...
apply stupid in inv_l0. rewrite inv_l0. rewrite compose_asso_2. rewrite compose_with_Id. apply functional_extensionality. exact inv_l1.
...
apply stupid in inv_r1. rewrite inv_r1. rewrite compose_with_Id. apply functional_extensionality.
exact inv_r0.
Defined.
...
Proof. (* m = n ~=~Eq_5 m n = Eq_5 sj m sj n ~=~ Sj m = Sj n *)
intros. apply (equiv_transitivity (m = n)  (Eq_5_b m n) (Sj m = Sj n)).
- apply Nat_equal.
- rewrite (Eq_succ m n) . apply equiv_refl. apply (Nat_equal (Sj m) (Sj n)).
Defined.

Lemma encode_5_2_nat (m n : nat) : ( m = n) -> (S m = S n).
Proof.
intro. destruct X. destruct m.
- apply refl.
- apply refl.
Defined.

Lemma decode_5_2_nat (m n : nat) : (S m = S n) -> (m = n).
Proof.
intros. apply eq_add_S. apply X.
Defined.

Lemma eg_nat (m n : nat): (m = n) ~=~ (S m = S n).
Proof.
exists (encode_5_2_nat m n). exists (decode_5_2_nat m n) (decode_5_2_nat m n). 
- unfold "=~". intros. unfold "¤". unfold decode_5_2_nat. unfold eq_add_S. unfold encode_5_2_nat. simpl. unfold Id.  (* montrer que Ap f (ap g p) = ap ( f o g ) p
Ap succ (zp pred x) = ap (succ o pred) x *) unfold ap. unfold pred. unfold refl. 
Admitted.
