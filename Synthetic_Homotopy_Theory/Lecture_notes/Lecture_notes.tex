...
Such that $e$, $\_\cdot\_$ and $\_^{-1}$ obey the laws of a group. There is a geometrical intuition, which should be transparent from our choice of names for the elements. %ing elements points or paths. 
...
\[p_1 : A\times B\r A\]
\[p_2 : A\times B \r B\]
 bu $p_1(x,y) :\equiv x$ and $p_2(x,y) :\equiv y$. %We denote $p_1(z)$ by $z.1$, and $p_B(z)$ by $z.2$.
...
\sse{Homotopies between functions}
...
We see that the non-dependent case is a special case of the dependent one. We will see soon that an homotopy between two functions is the same thing as an equality between these functions.



\sse{Identity types in the universe: the univalence axiom}
...
But the  previous lemma does not hold for $\mathrm{isQuasiEquiv}$, so it is not equivalent to $\mathrm{isEquiv}$. %There is no equivalence between the two 
...
The proof of this lemma uses the soon to be introduced \emph{univalence axiom} (together with the notion of equivalence). It is a bit tricky, so we admit it. It can be found in section 4.9 of \cite{hottbook}.
...
\[(z,z':A\times B)\ \r\ \big(z=_{A\times B} z' \simeq \mathrm{Eq}_{A\times B}(z,z')\big)\]
...
\begin{eqnarray}
\encode(\refl_{(x,y)})\,  &:\equiv& \, (\refl_x,\refl_y) \label{encodeprod}
\end{eqnarray}
%by definition.
...
\[\decode : (x=_Ax')\times (y=_By') \r (x,y)=_{A\times B}(x',y') \label{decodeprod}\]
By induction on the product and two path inductions, it is enough to define:

\begin{eqnarray}
\decode(\refl_x,\refl_y) \, &:\equiv& \, \refl_{(x,y)} \label{decodeprod}
\end{eqnarray}
...
but this is true by \ref{encodeprod} and \ref{decodeprod}.
...
Where $\ua$ is the map from $A=_\U B$ to $A\simeq B$. It is assumed to be an equivalence by the univalence axiom.
...
\[(e:A\simeq A')\times (f=_{A\r B} g \circ e)\]
...
%\[\tr^{\Gl Z.Z\r X}_\epsilon(p_1) =_{X\r Y} f \]
\end{proof}
...
\[\fib_f(y) \, :\equiv \, (x:X)\times f(x) =_Y y\]
\end{definition}

Note that there is a canonical map $p_1:\fib_f(y) \r X$.

\begin{remark}
Here the set-theoretic perspective is that $\fib_f(y)$ is what is usually denoted $f^{-1}(y)$. We will see that this analogy fails to explain some phenomena. A far better analogy is with the \emph{homotopy fibers} in traditional algebraic topology. %, for example the fibers of $p_1$ can complicated.
\end{remark}
...
\begin{lemma}
\label{auxiliaryGrothendieckCorrespondance}
Assume given $Y:\U$ with $P:Y\r \U$ and $b:Y$. Then we have:
\[\fib_{p_1}(b) \ \simeq\ P(b)\]
\end{lemma}
\begin{proof}
We just construct an equivalence between $\fib_{p_1}(b)$ which is defined as:
\[\big(z:(y:Y)\times P(y)\big) \times \big(p_1(z) =_Y b\big)\]
and $P(b)$.
\begin{itemize}
\item We define $\phi : P(b) \r \big(z:(y:Y)\times P(y)\big)\times \big(p_1(z) =_Y b\big)$ by:
\[\phi(r) \, :\equiv\, ((b,r),\refl_b)\]
\item We need to define: 
\[\psi : \big(z:(y:Y)\times P(y)\big)\times \big(p_1(z) =_Y b\big) \r P(b)\] 
...
\begin{exercise}
Define the successor function $\lambda(x:\mathbb{Z}).\, x+1$ from $\mathbb{Z}$ to $\mathbb{Z}$. Prove it is an equivalence the predecessor function $\lambda(x:\mathbb{Z}).\, x-1$ as inverse.
\end{exercise}
...
For $t:P(\base)$ and 
%\[\apd\]
\[e : \tr^P_\loop(t) =_{P(\base)} t\]% In this case:
%\[\apd_f(\loop) = \epsilon\] 
\end{itemize}
\end{definition}

This is a type freely generated by a point $\base$ and an identity proof $\loop$. As a space this correspond to a point $b$ with a path from this point to itself added, i.e. a circle. So we call $S^1$ the circle. Now we spell out the non-dependent elimination principle for the circle.

\begin{lemma}
\label{nondepinductioncircle}
Assume given $A:\U$ with $t:A$ and $e:t=_At$. Then we have a map $f : S^1\r A$ such that:
\[f(\base)\, :\equiv\, t\]
\[\ap_f(\loop) \, =\, e\]
\end{lemma}
\begin{proof}
We use the dependent elimination principle on the constant family:
\[P : (x:S^1)\mapsto A\]
See Lemma 6.2.5 in the HoTT book for the details.
\end{proof}
...
\begin{definition}
Using Lemma \ref{nondepinductioncircle} we define $\Eq_{S^1} : S^1 \r \U$ such that:
\[\Eq_{S^1}(\base) \, :\equiv\, \mathbb{Z}\]
\[\ap_{\Eq_{S^1}}(\loop) \, =\, \ua^{-1}(\Gl(x:\mathbb{Z}).\, x+1)\] %: \mathbb{Z}=_\U\mathbb{Z}\] 
...
\end{lemma}
\begin{proof}
We prove that for any $B:A\r \U$ and $p:x=_Ay$ we have:
\[\tr^B_p = \ua(\ap_B(p))\] 
by path induction. But then we know that: 
\[\ap_{\Eq^{S^1}}(\loop) = \ua^{-1}(\Gl m.m+1)\] 
so we can conclude.
\end{proof}
...
using the fact that $\tr_q^{\Gl x.a=_Ax}(p) = p\cdot q$ for any type $A$. This last statement is can be deduced from the definition of $\loop^n$.
\end{itemize}
\end{proof}
...
Using induction on $n:\mathbb{Z}$ we can conclude. The details are left as an exercise.
\end{proof}
...
\[\tr_\loop(H) =_{(n:\mathbb{Z}) \r \encode(\loop^n) =_{\mathbb{Z}} n} H\]
...
\[\tr_\loop(H)(n) =_{\encode(\loop^n) =_{\mathbb{Z}} n} H(n)\]
...
First we define the notion of contractibility. Intuitively a space is contractible if it can be deformed continuously to one of its points (e.g. a disk, $\mathbb{R}$, and so on). %A contractible $\infty$-groupoid is an 
...
%where $p_1$ is the canonical map from $(x:A)\times P(x)$ to $A$.
\end{proposition}
\begin{proof}
...
where:
\begin{itemize}
\item The type $\fib_g(*)$ is given in Definition \ref{defFibre}. It is pointed by $(*_X,*_g)$.
...
But this is Lemma \ref{auxiliaryGrothendieckCorrespondance}. Then we need: 
...
\[(V : \U_*)\times (V\r_* Y)\]%$pointed maps to $Y$. 
\end{proposition}
\begin{proof}
We have that: 
...
By our analysis of identity types in set-truncation in TD2, this is equivalent to $x:X$ such that $|f(x)=y|$. Using induction on the truncation again, we can assume given $x:X$ such that $f(x)=y$, and we can conclude.
...
\end{itemize}
Both these facts come from the reluctance from some teachers in algebraic topoplogy to use homotopy fibers rather than usual fibers. These notes\footnote{\url{https://www.math.univ-paris13.fr/~vallette/download/Homotopy\%20Theories.pdf}} by Johan Leray and Bruno Valette are a notable exception. % \url{https://www.math.univ-paris13.fr/~vallette/download/Homotopy\%20Theories.pdf}. %In HoTT we can only use homotopy fibers.
\end{remark}
...
We give one last application of our alternative definition of groups. %our As a last f application of our alternative definition  we give an application to ($\infty$-)group theory.
...
In an introductory class to algebraic topology, cohomology theories with coefficient in an abelian group are defined. We will define them with coefficient in a spectrum, a generalisation of abelian $\infty$-groups.% are usually defined 
...
Now we present some properties of cohomology theories, which allow us to compute them efficiently. Elilenberg and Steenrod \emph{defined} cohomology theories using these properties. % Eilenberg-Steenrod axioms for cohomology theories.
...
\item Using Grothendieck correspondence and our alternative definition of groups, we were able to prove four important results from homotopy theory relatively easily. %Moreover we were able to emphasized how similar they are.
